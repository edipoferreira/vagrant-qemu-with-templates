...
ssh_config_fullpath_destination = configuration['ssh_conf_file']['destination_full_path']
cleanup_tasks = configuration['shell_scripts']['trigger_cleanup']['script_path']
...
Vagrant.configure(VAGRANT_API_VERSION) do |config|
  ...
  File.open(ssh_config_fullpath_destination, "w") do |file|
    file.write(ssh_config)
  end  


  config.trigger.after :destroy do |trigger|
    trigger.name = "Cleaning up"
    trigger.run = {inline: "bash #{cleanup_tasks}"}
  end
